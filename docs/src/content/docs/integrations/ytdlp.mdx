---
title: yt-dlp
description: YouTube and web-backed downloads for flows and playlists.
---

yt-dlp is the built-in fallback download source. It searches YouTube for artist and title matches, and it can download an exact SoundCloud track selected by URL.

yt-dlp extracts audio with ffmpeg. Aurral stages the audio outside your media paths, validates and tags it, and then imports it into your Downloads Folder.

The default staging path is `/config/_staging`. To use another disk, mount it in the Aurral container and set **Settings > Download Clients > yt-dlp > Staging path**. Keep this path outside every media-server library.

You do not need a separate daemon. The Docker image includes `yt-dlp` and `ffmpeg`.

For a bare-metal installation, install both programs on the host. Make sure that `yt-dlp` is on `PATH`.

## Setup

1. Open **Settings > Download Clients > yt-dlp**.
2. Leave **Enable yt-dlp** on (default).
3. If necessary, set **Source priority** or **Staging path**.
4. Select **Test connection**.
5. Settings save automatically after each change.

The yt-dlp default priority is `50`. The slskd default is `10`. The Usenet default is `20`.

## Import a SoundCloud track

1. Open **Library > Playlists**.
2. Select **Create playlist > Import playlist > SoundCloud URL**.
3. Paste a public SoundCloud track URL and select **Inspect**.
4. Review the resolved title, uploader, and duration.
5. Choose an existing playlist or name a new playlist, then select **Download track**.

Aurral downloads the inspected URL directly instead of searching for a similarly named track. The resulting file is stored in the Downloads Folder, tagged for the media library, and reused by later playlists rather than downloaded again.

Only download tracks that you have permission to save. Playlist, profile, private, blocked, and non-SoundCloud URLs are rejected.

## Behavior

Aurral tries download sources in priority order. With default settings, Aurral tries configured slskd and Usenet sources first.

yt-dlp downloads tracks that the other sources cannot find. It can also be the only download source.

Aurral ranks results by title, artist, duration, and noise filters. It skips karaoke, live loops, and similar unwanted results.

Aurral validates downloaded audio with the same identity, duration, and media checks that it uses for other sources.

Search-based downloads also apply the global Quality Profile. An explicitly selected SoundCloud track keeps recognized, playable source audio even when its measured bitrate is below the automatic profile floor; there is no alternate candidate to upgrade during an exact import.

After validation, Aurral writes the playlist's title, artist, album, album artist, year, and track number into the audio file. This metadata lets tag-based libraries such as Navidrome organize yt-dlp downloads correctly.

When a downloaded file matches the requested track but its duration differs significantly, Aurral retains it under **Activity > Queue** for preview, approval, or denial.

Audio from YouTube is usually lossy. Prefer slskd or Usenet when you want higher quality.

yt-dlp can supply the first acceptable file for a track. Automatic and manual upgrade searches use slskd or Usenet, not yt-dlp.

## Download source priority

See [Usenet](/integrations/usenet/#download-source-priority-and-fallback) for how priority and fallback work across sources.
